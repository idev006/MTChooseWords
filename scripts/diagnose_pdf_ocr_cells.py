...
def main() -> int:
    ...
    payload = {
        ...
        "accept_count": sum(1 for row in rows if row.status == "ACCEPT"),
        "review_count": sum(1 for row in rows if row.status == "REVIEW"),
        "coverage": coverage,
        "meets_min_coverage": coverage is None or coverage >= args.min_coverage,
    ...
    print(json.dumps({
        ...
        "meets_min_coverage": payload["meets_min_coverage"],
    }, ensure_ascii=False))
    return 0
